...
function saveState() {
  ...
  fs.writeFileSync(settingsPath, JSON.stringify({ enabled, settings }, null, 2));
}
...
function emitClick(click) {
  ...
  const point = click.test ? click : screen.screenToDipPoint({ x: Number(click.x), y: Number(click.y) });
...
function startMouseHook() {
  ...
  hookProcess = spawn('powershell.exe', [
    '-NoProfile',
    '-ExecutionPolicy',
    'Bypass',
    '-File',
